...
# ────────────────────────────────────────────────
#   КОНФИГУРАЦИЯ ПОД СИНТЕТИЧЕСКИЙ ДАТАСЕТ
# ────────────────────────────────────────────────
...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    """Минимальная разумная очистка"""
    df = df.copy()

    # Убираем совсем нереалистичные значения (хотя в синтетике их почти нет)
...
def preprocess_and_train(
    ...
    # PowerTransformer на цену (очень полезно для скошенных цен)
    ...
    with mlflow.start_run(run_name="SGD_Synthetic_250"):
        ...
        # Сохраняем трансформер цены отдельно
